...
1. **개인 브랜치에서 작업**
   ```powershell
   git checkout 배민  # 또는 자신의 이름 브랜치
   git pull origin 배민
   # ... 수정 ...
   git add .
   git commit -m "커밋 메시지"
   git push origin 배민  # develop이 아닌 개인 브랜치로 푸시
   ```

2. **develop으로 병합**
   - 로컬: `git merge 배민` (develop 브랜치에서)
   - 또는 GitHub에서 `배민 → develop` PR 생성 후 병합
   - **충돌 발생 시**: 같은 파일을 수정한 사람끼리 조율

3. **최종**: develop → main 병합
